...
- Keep Claude provider context guardrails explicit in dedicated settings: disable nonessential traffic and set the auto-compaction calculation window when the cataloged profile requires it. Use a settings-level effort default when native `/model` and `/effort` controls must remain available; do not lock those controls with the higher-precedence `CLAUDE_CODE_EFFORT_LEVEL` environment variable. Document the exact values, scope, and residual context-overflow risk.
